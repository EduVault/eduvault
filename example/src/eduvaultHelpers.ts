...
export async function setupApp() {
  ...
  const devPersonRegister = await personRegister({ username, password });
  console.log({ devPersonRegister });
  const dev = await devVerify(APP_SECRET, username);
  console.log({ dev });
  const appInfo = await appRegister(username, password, 'test app', 'a testing app');
  console.log({ appInfo });
